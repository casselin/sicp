...
(#%require "table.rkt")
;; Exercise 4.78
; Query language as a nondeterministic program
...
(define (disjoin disjuncts frame)
  (let ((disjunct (an-element-of disjuncts)))
    (qeval (disjunct) frame)))
(put 'or 'qeval disjoin)

#| This requires the if-fail special form from exercise 4.52
(define (negate operands frame)
  (if-fail (qeval (negated-query operands) frame)
           frame
           (amb)))
